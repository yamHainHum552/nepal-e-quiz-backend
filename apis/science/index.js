...
export const CreateScience = async (req, res) => {
  try {
    ...
    const newQuestion = new ScienceQuestions({
      id: data.id,
...
export const DeleteScience = async (req, res) => {
  try {
    ...
    const deletedQuestion = await ScienceQuestions.findOneAndDelete({
      id,
    });
    if (!deletedQuestion) {
...
export const UpdateScience = async (req, res) => {
  try {
    ...
    const existingQuestion = await ScienceQuestions.findOne({
      id: Number(id),
    });
    if (!existingQuestion) {
      return res.status(404).json({ message: "Questions doesn't exists" });
    }
    const updatedQuestion = await ScienceQuestions.findOneAndUpdate(
      { id },
