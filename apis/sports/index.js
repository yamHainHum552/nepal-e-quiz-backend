import { SportsQuestions } from "../../schemas/index.js";
...
export const CreateSports = async (req, res) => {
  try {
    ...
    const newQuestion = new SportsQuestions({
      id: data.id,
...
export const DeleteSports = async (req, res) => {
  try {
    const { id } = req.body;
    const deletedQuestion = await SportsQuestions.findOneAndDelete({
      id,
    });
    if (!deletedQuestion) {
...
export const UpdateSports = async (req, res) => {
  try {
    ...
    const existingQuestion = await SportsQuestions.findOne({
      id: Number(id),
    });
    if (!existingQuestion) {
      return res.status(404).json({ message: "Questions doesn't exists" });
    }
    const updatedQuestion = await SportsQuestions.findOneAndUpdate(
      { id },
